  ...
  async getCountersViewByUser(userId: string): Promise<CounterRecordList[]> {
    ...
    const list: CounterRecordList[] = snaps.docs
      ...
      return {
        ...
        leftSetsWon: data.games?.filter(g => g.leftValue > g.rightValue && (data.isFinished || g.id !== data.currentGameId)).length ?? 0,
        rightSetsWon: data.games?.filter(g => g.rightValue > g.leftValue && (data.isFinished || g.id !== data.currentGameId)).length ?? 0,
